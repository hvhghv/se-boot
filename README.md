...
#### 脚本格式
文件名为`<priority>_<timeout>_<name>.sh`, 其中priority与timeout必须为2位数。priority值小的优先执行，值大的会等待值小的执行完或者超时后才执行，其值范围为00-99。timeout以秒为单位，范围为00-99，超时后停止等待（但不会杀死）该脚本并选取其他脚本执行。
...
- 当然，也可以添加好头文件路径后直接编译所有`se-boot-src`下所有的`*.c`文件
